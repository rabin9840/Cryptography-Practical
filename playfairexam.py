    ...
    for i in range(0,len(plainText),2):
        if(i<len(plainText)-1):
            if plainText[i]==plainText[i+1]:
                #when the two correponding text are same
                #seperate the two letters with x
                plainText=plainText[:i+1]+'x' +plainText[i+1:]

    #adding x if odd no of element to make the number of letter even
...
#     exit()
# else:
#     print("Wrong choice!Please select the correct key.")
